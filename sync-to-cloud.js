...
class DatabaseSync {
  ...
  async syncCollection(collectionName) {
    try {
      ...
      // 获取本地集合数据
      const localCollection = this.localConnection.collection(collectionName);
      ...
      // 获取云端集合
      const cloudCollection = this.cloudConnection.collection(collectionName);
  ...
  async syncAll() {
    try {
      ...
      console.log(`📝 需同步集合: ${COLLECTIONS.join(', ')}\n`);
