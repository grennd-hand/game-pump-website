...
function LeaderboardContent() {
  ...
  return (
    <PageTransition pageKey="leaderboard-page">
        <Header lang={lang} setLang={setLang} />
        
        {/* 页面头部 */}
        <motion.section 
          initial={{ opacity: 0, x: 250, rotateY: 25 }}
          animate={{ opacity: 1, x: 0, rotateY: 0 }}
          transition={{ duration: 1, delay: 0.3, type: "spring", stiffness: 160 }}
        ...
        >
          <div className="max-w-6xl mx-auto px-6">
            ...
            >
              <motion.h1 
                key={`leaderboard-title-${lang}`}
                initial={{ opacity: 0, rotateY: 45, scale: 0.7 }}
                animate={{ opacity: 1, rotateY: 0, scale: 1 }}
                transition={{ duration: 0.8, delay: 0.7, type: "spring", stiffness: 250 }}
                className="text-3xl sm:text-4xl md:text-5xl font-bold font-retro text-retro-green mb-4" 
                style={{ textShadow: '0 0 10px currentColor' }}
              >
                🏆 {pageInfoMap[lang].pageTitle}
              </motion.h1>
              <motion.p 
                key={`leaderboard-desc-${lang}`}
        ...
        {/* 增强版排行榜内容 */}
        <motion.div
          initial={{ opacity: 0, x: 180, scale: 0.9, rotateX: 10 }}
          animate={{ opacity: 1, x: 0, scale: 1, rotateX: 0 }}
          transition={{ duration: 1.1, delay: 0.6, type: "spring", stiffness: 140 }}
        >
          <EnhancedLeaderboard />
        </motion.div>
